...
For this visual, I filtered for a single state (Georgia), and 

```{r}
# states <- c("Alabama", "California", "Colorado", "Michigan", "New York", "Texas")
states <- c("Georgia")

p_waffle <- ggplot(disease_2 %>% filter(state %in% states, year %in% c(1976:1995))) +
...
```{r}
p_line <- ggplot(disease_2 %>% filter(state %in% states, year %in% c(1940:2000))) +
...
# Part 2: International Murders

```{r}
names(guns) <- c("country", "count")
ggplot(guns) + geom_bar(aes(x = country, y = count), stat = "identity", fill = "red")
```
